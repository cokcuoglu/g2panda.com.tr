...
async function testExpense() {
    ...
    try {
        ...
        // 2. Try to Insert a Test Expense
        console.log('\n🧪 Testing Expense Insertion...');

        await client.query('BEGIN');

        // Note: Default document_type is 'receipt' in backend code
        const insertRes = await client.query(`
            INSERT INTO transactions (
                user_id, type, amount, transaction_date, description, 
                expense_type, document_type, is_tax_deductible
            ) 
            VALUES ($1, 'expense', 10.00, NOW(), 'Test Expense Check', 'other', 'receipt', true)
            RETURNING id, document_type, amount
        `, [userId]);

        console.log('✅ Success! Inserted Expense:', insertRes.rows[0]);

        await client.query('ROLLBACK'); // Rollback so we don't pollute data
